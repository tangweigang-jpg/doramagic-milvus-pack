...
- Installation risk requires verification (medium): Upgrade or migration may change expected behavior: client/v2.6.3 Suggested check: Before packaging this project, run the relevant install/config/quickstart check for: client/v2.6.3. Context: Observed during version upgrade or migration.
...
- Installation risk requires verification (medium): May increase setup, validation, or first-run risk for the user. Suggested check: Reproduce the official install and quickstart path in an isolated environment.
- Configuration risk requires verification (medium): Upgrade or migration may change expected behavior: client/v2.6.4 Suggested check: Before packaging this project, run the relevant install/config/quickstart check for: client/v2.6.4. Context: Source discussion did not expose a precise runtime context.
